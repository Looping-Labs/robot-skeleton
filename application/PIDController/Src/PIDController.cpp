...
namespace controller {
  ...
  PIDController::PIDController(float Kp, float Ki, float Kd, uint32_t dt_ms, float min_output, float max_output)
    ...
    ESP_LOGI(TAG, "PID Controller created with Kp=%.3f, Ki=%.3f, Kd=%.3f, dt=%.3fs", Kp, Ki, Kd, dt);
  }
  ...
  float PIDController::compute(float error) {
    ...
    ESP_LOGD(TAG, "PID computation: error=%.3f, P=%.3f, I=%.3f, D=%.3f, output=%.3f", error, p_term, i_term, d_term, output);
